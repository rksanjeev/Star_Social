...
class UsersPost(generic.ListView):
    model = models.Post
    template_name = 'posts/user_post_list.html'

    def get_queryset(self):
        try:
            self.post_user =  User.objects.prefetch_related('posts').get(username__iexact= selft.kwars.get('username'))
        except:
            raise Http404
        else:
...
class PostDetails(SelectRelatedMixin, generic.DetailView):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(user__username__iexact = slef.kwargs.get('username'))
